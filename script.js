// script.js (v4.0 - 合併最終版)

// =======================================================
// 教學功能物件 (原 tutorial.js 的內容)
// =======================================================
const tutorial = {
    moveDescriptions: {
    ...
    isValidMove(fromRow, fromCol, toRow, toCol, board) {
        ...
        switch (pieceChar) {
            ...
            case '俥': case '車':
                ...
                if (fromRow === toRow) { for (let c = Math.min(fromCol, toCol) + 1; c < Math.max(fromCol, toCol); c++) if (board[fromRow][c]) return false; } 
                else { for (let r = Math.min(fromRow, toRow) + 1; r < Math.max(fromRow, toRow); r++) if (board[r][fromCol]) return false; }
                return true;
            case '傌': case '馬':
            ...
            case '炮': case '砲':
                if (fromRow !== toRow && fromCol !== toCol) return false;
                let screenCount = 0;
                if (fromRow === toRow) { for (let c = Math.min(fromCol, toCol) + 1; c < Math.max(fromCol, toCol); c++) if (board[fromRow][c]) screenCount++; } 
                else { for (let r = Math.min(fromRow, toRow) + 1; r < Math.max(fromRow, toRow); r++) if (board[r][fromCol]) screenCount++; }
