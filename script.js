...
const game = {
    ...
    elements: {
        ...
        undoBtn: document.getElementById('undo-button') // 悔棋按鈕的參照
    },
    ...
    init() {
        ...
        this.elements.undoBtn.addEventListener('click', this.undoMove.bind(this)); // 綁定悔棋事件
    },
    ...
    resetGame() {
        ...
        this.state.moveHistory = []; // 重置遊戲時清空歷史紀錄
        this.renderBoard();
    ...
    // --- 悔棋函式 ---
    undoMove() {
        if (this.state.isGameOver || this.state.moveHistory.length === 0) {
            return;
        }
    ...
    showWarning(reason) {
        ...
        setTimeout(() => {
            if (this.elements.info.textContent === message) {
